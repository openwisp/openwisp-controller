  ...
  $.get(whoIsInfoUrl).done(function (data) {
    parentDiv.after(
      `<div>These details are based on last public IP: ${data.ip_address}</div>` +
        ...
        `<details id="who_is_details"><summary><span class="who_is_globe"></span><div><span>Additional Details</span><span class="mg-arrow"></span></div></summary><div><p>ASN : ${data.asn}</p><p>Timezone : ${data.timezone}</p><p>Address : ${data.address}</p><p>CIDR : ${data.cidr}</p></div></details>`,
    );
  });
});
