...
class AbstractTestAdmin(
    CreateConfigTemplateMixin, TestVpnX509Mixin, TestAdminMixin, TestOrganizationMixin
):
    """
    ...
    def test_change_device_clean_templates(self):
        ...
        path = reverse('admin:config_device_change', args=[d.pk])
    ...
    def test_download_device_config(self):
        ...
        path = reverse('admin:config_device_download', args=[d.pk])
        response = self.client.get(path)
    ...
    def test_download_template_config(self):
        t = self.template_model.objects.first()
        path = reverse('admin:config_template_download', args=[t.pk])
        response = self.client.get(path)
        self.assertEqual(response.get('content-type'), 'application/octet-stream')

    def test_preview_template(self):
        template = self.template_model.objects.get(name='radio0')
        path = reverse('admin:config_template_preview')
        data = {
            'name': template.name,
    ...
    def test_change_device_404(self):
        path = reverse('admin:config_device_change', args=[self.device_model().pk])
        response = self.client.get(path)
        self.assertEqual(response.status_code, 404)

    def test_change_device_malformed_uuid(self):
        path = reverse('admin:config_device_change', args=['wrong'])
        response = self.client.get(path)
        self.assertEqual(response.status_code, 404)
    ...
    def test_uuid_field_in_change(self):
        ...
        c = self._create_config(device=d, backend=t.backend, config=t.config)
        path = reverse('admin:config_device_change', args=[c.device.pk])
        response = self.client.get(path)
        self.assertEqual(response.status_code, 200)
    ...
    def test_download_vpn_config(self):
        v = self._create_vpn()
        path = reverse('admin:config_vpn_download', args=[v.pk])
        response = self.client.get(path)
        self.assertEqual(response.get('content-type'), 'application/octet-stream')

    def test_preview_vpn(self):
        v = self._create_vpn()
        path = reverse('admin:config_vpn_preview')
        data = {
            'name': v.name,
            'backend': v.backend,
            'host': v.host,
    ...
    def test_add_vpn(self):
        path = reverse('admin:config_vpn_add')
        response = self.client.get(path)
        self.assertContains(
    ...
    def test_error_if_download_config(self):
        d = self._create_device()
        res = self.client.get(reverse('admin:config_device_change', args=[d.pk]))
    ...
    def test_context_device(self):
        ...
        url = reverse('admin:config_device_context', args=[device.pk])
    ...
    def test_context_vpn(self):
        vpn = self._create_vpn()
        url = reverse('admin:config_vpn_context', args=[vpn.pk])
        response = self.client.get(url)
    ...
    def test_context_template(self):
        template = self._create_template()
        url = reverse('admin:config_template_context', args=[template.pk])
        response = self.client.get(url)
